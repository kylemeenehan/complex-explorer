...
float getIterationCount(float px, float py) {
  ...
  float cx = uPosX + scale(px, 0.0, uCanvasWidth, -uSizeX / 2.0, uSizeX / 2.0);
  float cy = uPosY + scale(py, uCanvasHeight, 0.0, -uSizeY / 2.0, uSizeY / 2.0);
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    // Complex quadratic formula: z = z^2 + c
    // z^2 = (x + yi)^2 = x^2 - y^2 + 2xyi
    float xtemp = zx * zx - zy * zy + cx;
    float ytemp = 2.0 * zx * zy + cy;
    zx = xtemp;
    zy = ytemp;
    
    if (sqrt(zx * zx + zy*zy) > 2.0) break;
    iter += 1.0;
  }
  return iter;
}
